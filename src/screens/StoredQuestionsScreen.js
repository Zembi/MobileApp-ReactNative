...
import {
    StyleSheet, Text, View, SafeAreaView, Platform, Animated, ImageBackground,
    FlatList, Button, TouchableHighlight, Alert, InteractionManager, useFocusEffect
} from 'react-native';
...
export default class StoredQuestionsScreen extends React.Component {
    constructor(props) {
        super(props);
    ...
    updateStoreQues(newStoredQs) {
        this.setState({
            storedQuestionsId: newStoredQs.map(dbData => dbData.id),
            emptyQs: newStoredQs.length === 0 ? true : false,
            storedQuestionsFromJson: this.questionsFromJson.questions.filter((question) =>
    ...
    getSavedTable() {
        this.DB.transaction(
            (tx) => {
                tx.executeSql(
                    `SELECT * FROM ${this.tableOfData} WHERE SavedQ = TRUE`,
                    [],
                    (tx, results) => {
    ...
    insertIntoTable(id, status) {
        this.DB.transaction(
            (tx) => {
                tx.executeSql(
                    `INSERT INTO ${this.tableOfData} (Id, SavedQ) 
                     SELECT ?,? 
    ...
    componentDidMount() {
        ...
        this.getSavedTable();
        // console.log(this.nav.getState().routes);
    }
    ...
    informChildBtnIfQueEmpty(idToDelete) {
        ...
        this.getSavedTable();
        this.setState({
            storedQuestionsFromJson: this.state.storedQuestionsFromJson.filter(question => question.id !== idToDelete),
        })
    }

    actionToHeader() {
        this.state.storedQuestionsFromJson.map(question => {
        ...
        return (
            <SafeAreaView style={[styles.container]} >
                ...
                <Animated.View style={[
                    ...
                    <Text style={styles.movedInSumOfQuesText}>Σύνολο αποθηκευμένων ερωτήσεων: {this.state.storedQuestionsFromJson.length}</Text>
                </Animated.View>
                <View style={styles.listOfQuestionsWrapper}>
                    {this.state.emptyQs ?
                        ...
                        :
                        <FlatList
                            data={this.state.storedQuestionsFromJson}
